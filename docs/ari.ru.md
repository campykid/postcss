...
## `postcss` function
 ...
 `postcss` функция которая является главной точкой входа в PostCSS.

```js
var postcss = require('postcss');
```

Те кто использует TypeScript вместе с [ES6 compile target], могут импортировать
 PostCSS API так:
...
Возвращает новый [`Processor`] экземпляр к которому будут применяться `plugins`
 CSS процессоры.
...
* `plugins (array)`: список плагинов которые будут применены.

Плагины могут так же быть подключены как [`Processor#use`] метод.
Дополнительную информацио о формате плагинов можно найти ниже.

### `postcss.parse(css, opts)`

Анализирует `css` and возврщает новый `Root` узел, который содержит CSS узлы.
